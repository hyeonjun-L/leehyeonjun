...
그런데 한 가지가 안 맞습니다. `0.9996`을 그대로 곱하면 차이가 **3.508m**여야 하는데, 측정값은 **3.300m**입니다. 0.2m가 남아요.

```text
8770.914 m × (1 − 0.9996) = 3.508 m   ← 0.9996만 쓰면
                            3.300 m   ← 실제 측정
```

서울이 **기준선 바로 위에 있지 않아서**입니다. 두 점의 한가운데는 경도 127.00275도, 기준선에서 0.5도쯤 서쪽이에요. 방금 표에서 봤듯 기준선에서 벗어나면 조금씩 늘어나기 시작하니, 서울은 0.04%를 다 줄이지 못하고 **조금 덜 줄어든** 자리입니다. 그 "조금 덜"은 네 단계로 계산할 수 있습니다.

```text
① 기준선에서 얼마나 떨어졌나
   경도 차      127.00275° − 127.5°                  = −0.49725°
   위도 보정    −0.49725° × cos(37.5321°) = × 0.7930 → −0.39433°
   라디안으로   −0.39433° × π / 180                  → X = −0.006882

② 그 자리에서 얼마나 늘어나나
   1 + X² / 2 = 1 + 0.0000237                        = 1.0000237

③ 일부러 줄여 둔 0.9996과 합친다
   0.9996 × 1.0000237                                = 0.999623673

④ 거리에 대 본다
   8770.914 m × (1 − 0.999623673)                    = 3.3007 m   ← 측정 3.2997 m
```

한 줄씩 풀면 이렇습니다.

- **①** 같은 경도 0.5도라도 **위도에 따라 실제 폭이 다릅니다.** 경도선은 북극과 남극에서 한 점으로 모이니까, 적도에서 넓던 경도 1도가 위도 37.5도에서는 약 0.79배(`cos 37.5°`)로 좁아져요. 그 폭으로 바꾼 다음 식이 받는 단위인 **라디안**으로 옮긴 게 `X`입니다. 마이너스는 서쪽이라는 뜻일 뿐이고요.
- **②** 원통을 딱 붙였을 때 기준선에서 `X`만큼 떨어진 자리가 얼마나 늘어나는지를, 기준선 근처에서 계산하는 식이 `1 + X²/2`입니다. 위 표의 `딱_붙였을_때` 열이 바로 이 값이에요. `X`를 **제곱**하니 동쪽이든 서쪽이든 똑같이 늘어나고, 멀어질수록 빠르게 커집니다. 서울은 0.5도밖에 안 떨어져 있어서 겨우 **0.0024%** 늘어났습니다.
- **③** 거기에 5179가 일부러 줄여 둔 `0.9996`을 곱하면 **서울 자리의 실제 눈금**이 나옵니다. 위 표에서 `딱_붙였을_때`에 0.9996을 곱한 게 `5179_밀어_넣음`이었던 것과 같은 계산이에요. 결과 `0.999624`는 앞에서 두 거리를 나눠 역산한 값과 같습니다.
- **④** 그 눈금으로 거리를 줄여 보면 3.3007m. 측정한 3.2997m와 **1mm 차이**입니다.

3.3m는 결국 5179 설명서에 적힌 두 숫자, **축척계수 0.9996과 중앙자오선 127.5도**만으로 종이 위에서 계산되는 값이었습니다.
...
> 💡 그러니 **`ORDER BY`는 `<->`로, 출력은 `ST_Distance`로** 쓰는 게 맞습니다. 지난 편에서 쓴 패턴 그대로예요. `<->`는 인덱스를 태우려고 쓰는 것이고, 화면에 찍을 숫자는 따로 정확히 재면 됩니다. 정렬에 쓴 값을 그대로 출력하면 그건 타원체 거리가 아니거든요.
...
**두 점이 10,131km 떨어져 있습니다.** 평면 계산은 경도 `126.98`과 `-118.24`를 그냥 산술평균해서 `4.37`을 냈어요. 경도를 가로축으로 펼친 종이 위에서 두 도시를 자로 이으면 태평양 대신 중동과 북아프리카를 거쳐 서쪽으로 가니까, 종이 위에서는 맞는 답입니다. 다만 그 종이가 지구가 아니었을 뿐이죠.
...
geography는 왜 안 터질까요. 3절에서 경계상자가 8바이트 크다고 했던 그 이유입니다.

geography의 공간 인덱스는 경도·위도를 저장하지 않습니다. **지구 중심을 원점으로 하는 3차원 좌표로 바꿔서** 저장해요.
...
gis-21에서 저는 비교표에 **"함수 수 — 776개 대부분 / 일부만"** 이라고 적었습니다. "일부만"이 몇 개인지는 안 적었죠. 세어 봅니다.
...
gis-21의 비교표에는 **"속도 — 빠름 / 느림"** 이라는 칸도 있었습니다. 근거 없이요. 재 봅니다. 지난 편의 5179 테이블과, 같은 데이터를 geography로 옮긴 테이블을 각각 만들고 같은 조인을 돌립니다.
...
> **`::geography`는 좌표를 한 개도 바꾸지 않는다. 바이트를 그대로 둔 채 플래그 비트 하나를 켤 뿐이고, 그때부터 같은 이름의 함수가 다른 코드로 간다. 반대로 `ST_Transform(…, 5179)`은 한국 모양으로 깎은 자를 꺼내는 일이라, 서울에서 둘의 차이가 3.3m뿐인 건 두 방법이 비슷해서가 아니라 자가 여기에 맞게 깎였기 때문이다 — 그 3.3m조차 오차가 아니라 `+k=0.9996`이라는 설계값이다.**

오늘 나온 세 자를 한 자리에 놓으면 이렇게 됩니다.
...
| 재는 방법 | 평면에 펼친 뒤 직선 | 반지름 6,371,008.8m 공 위의 대권 | WGS84 타원체 위의 측지선 |
| 유효 범위 | **좌표계마다 정해져 있다** | 전 지구 | 전 지구 |

묻고 싶은 것에서 거꾸로 찾아가면 이렇습니다.

| 무엇을 하려는가 | 쓸 것 | 조심할 것 |
| --- | --- | --- |
| 한국 안에서 거리·넓이 | `ST_Transform(…, 5179)` | 한국 밖 좌표가 섞이면 **에러 없이 틀린다** |
| 한국 안, 변환 테이블 만들기 귀찮다 | `::geography` | 오늘 잰 조인에서 2.9배 느렸다 |
| 한국 밖 / 대륙 횡단 | `::geography` | 투영좌표계는 **자기 구역 밖에서 무효다** |
| 경도 180도 근처 | `::geography` | 평면은 반대쪽으로 돈다 |
| 면 연산 (`ST_Union`·`ST_Relate`…) | `geometry` | geography에 **없다** |
| 좌표 꺼내기 (`ST_X`·`ST_Y`) | `geometry` | geography에 **없다** |
| 가장 가까운 것 찾기 | `ORDER BY <->` + 출력은 `ST_Distance` | geography의 `<->`는 **구**로 잰다 |
